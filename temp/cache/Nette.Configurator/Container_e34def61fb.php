<?php
// source: /www/jvitasek.cz/test.jvitasek.cz/app/config/config.neon 

class Container_e34def61fb extends Nette\DI\Container
{
	...
	protected $meta = array(
		'types' => array(
			...
			'Models\Base' => array(
				1 => array('bookModel', 'homeModel', 'sailModel'),
			),
			'Models\Book' => array(1 => array('bookModel')),
			'Models\Home' => array(1 => array('homeModel')),
			'Models\Sail' => array(1 => array('sailModel')),
			...
			'App\Presenters\PlavbaPresenter' => array(array('application.1')),
			'App\Presenters\EmailsPresenter' => array(array('application.2')),
		...
		'services' => array(
			'application.1' => 'App\Presenters\PlavbaPresenter',
			'application.2' => 'App\Presenters\EmailsPresenter',
	...
	public function __construct()
	{
		parent::__construct(array(
			'appDir' => '/www/jvitasek.cz/test.jvitasek.cz/app',
			'wwwDir' => '/www/jvitasek.cz/test.jvitasek.cz',
			'debugMode' => TRUE,
			'productionMode' => FALSE,
			'environment' => 'development',
			'consoleMode' => FALSE,
			'container' => array('class' => NULL, 'parent' => NULL),
			'tempDir' => '/www/jvitasek.cz/test.jvitasek.cz/app/../temp',
		));
	}


	/**
	 * @return App\Presenters\PlavbaPresenter
	 */
	public function createServiceApplication__1()
	{
		$service = new App\Presenters\PlavbaPresenter;
		...
		$service->injectBase($this->getService('homeModel'), $this->getService('sailModel'), $this->getService('bookModel'));
	...
	/**
	 * @return App\Presenters\EmailsPresenter
	 */
	public function createServiceApplication__2()
	{
		$service = new App\Presenters\EmailsPresenter;
		$service->injectPrimary($this, $this->getService('application.presenterFactory'), $this->getService('routing.router'),
			$this->getService('http.request'), $this->getService('http.response'), $this->getService('session.session'),
			$this->getService('security.user'), $this->getService('latte.templateFactory'));
		$service->injectBase($this->getService('homeModel'), $this->getService('sailModel'), $this->getService('bookModel'));
	...
	{
		$service = new Nette\Application\PresenterFactory(new Nette\Bridges\ApplicationDI\PresenterFactoryCallback($this, 5, '/www/jvitasek.cz/test.jvitasek.cz/app/../temp/cache/Nette%5CBridges%5CApplicationDI%5CApplicationExtension'));
	 ...
	 * @return Models\Book
	 */
	public function createServiceBookModel()
	{
		$service = new Models\Book($this->getService('database.default.context'));
		return $service;
	}
	...
	{
		$service = new Nette\Caching\Storages\FileJournal('/www/jvitasek.cz/test.jvitasek.cz/app/../temp');
		return $service;
	}
	...
	{
		$service = new Nette\Caching\Storages\FileStorage('/www/jvitasek.cz/test.jvitasek.cz/app/../temp/cache', $this->getService('cache.journal'));
		return $service;
	}
	...
	{
		$service = new Nette\Database\Connection('mysql:host=127.0.0.1;dbname=jvitasek_cz_test', '30906_jvitasek', 'wu6raCet',
			array('lazy' => TRUE));
	 ...
	 * @return Models\Home
	 */
	public function createServiceHomeModel()
	{
		$service = new Models\Home($this->getService('database.default.context'));
		return $service;
	}
	...
	{
		return new Container_e34def61fb_Nette_Bridges_ApplicationLatte_ILatteFactoryImpl_latte_latteFactory($this);
	}
	...
	public function createServiceNette__latte()
	{
		...
		trigger_error('Service nette.latte is deprecated, implement Nette\Bridges\ApplicationLatte\ILatteFactory.',
			16384);
		$service->setTempDirectory('/www/jvitasek.cz/test.jvitasek.cz/app/../temp/cache/latte');
	 ...
	 * @return Models\Sail
	 */
	public function createServiceSailModel()
	{
		$service = new Models\Sail($this->getService('database.default.context'));
		return $service;
	}
...
final class Container_e34def61fb_Nette_Bridges_ApplicationLatte_ILatteFactoryImpl_latte_latteFactory implements Nette\Bridges\ApplicationLatte\ILatteFactory
{

	private $container;


	public function __construct(Container_e34def61fb $container)
	{
